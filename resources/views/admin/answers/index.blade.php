<x-admin-layout pageTitle="Hasil Pengisian">
  <form action="{{ route('admin.answers.filter') }}" method="POST">
      @csrf
      <div class="bg-white w-full mb-4 p-6">
          ...
          <button
              class="block px-4 py-2 w-[100px] ml-auto mr-0 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
              Terapkan
          </button>
      </div>
  </form>
                      ...
                      @if ($answer_status->answers->count() > 0)
                          <tr class="text-gray-700 dark:text-gray-400">
                              ...
                              <td class="px-4 py-3 flex space-x-2">
                                  <a href="{{ route('admin.answers.details', $answer_status->id) }}"
                                      class="text-sm bg-purple-400 px-2 py-1 rounded-md text-white font-semibold hover:bg-purple-500 transition-colors duration-200">Hasil</a>
                              </td>
                          </tr>
                      @endif
                  @endforeach
